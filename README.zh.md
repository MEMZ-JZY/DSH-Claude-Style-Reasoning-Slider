...
- **无推理元数据的模型也可管理** — 当 DSH 没有为当前模型返回 `reasoning`/`efforts`（例如部分 OpenRouter 或手写自定义条目）时，“推理等级”菜单仍会显示：面板中给出“未提供推理等级”的提示和安全可用的 **Default** 胶囊，而不是隐藏所有思考强度控件。
...
3. 在推理等级面板中：
   ...
   - 如果当前模型没有声明任何推理等级，面板会显示提示，并只提供 **Default** 胶囊。对于实际支持思考强度但配置中缺少 `reasoningEfforts` 的提供商（常见于自定义 OpenRouter 路由），需要在 DSH 的 `llm-pi-ai` 配置中补充等级后，滑块才会完整恢复。
