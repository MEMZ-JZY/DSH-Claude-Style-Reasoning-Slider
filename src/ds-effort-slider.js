...
class DsEffortSlider extends HTMLElement {
  ...
  // Non-elastic ease-out tween. Drives _setValue per frame so the thumb AND
  // the level colors glide together. Does not rely on @property registration,
  // which isn't honored in every environment, so this is deterministic.
  _animateTo(target) {
    this._cancelTimer("_tweenFrame");
    ...
    const step = () => {
      ...
      this._setValue(start + (target - start) * eased, { animateLabel: true, reflect: false });
      if (t < 1) {
        this._tweenFrame = effortTiming.raf(step);
      } else {
        this._tweenFrame = 0;
        this._setValue(target, { animateLabel: false, reflect: true });
        this._emit("change");
