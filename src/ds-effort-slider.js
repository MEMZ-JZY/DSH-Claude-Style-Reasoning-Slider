...
class DsEffortSlider extends HTMLElement {
  ...
  _drawRippleField(context, width, height, time, mode) {
    ...
    for (const c of cells) {
      ...
      if (dx > 1) continue;
      if (base > 0.3) continue; // 稀疏：约 7/10 粒子跳过

      // 随机闪烁：每颗粒子亮度随时间独立起伏
      const flicker = 0.5 + 0.5 * Math.sin(elapsed * 0.004 + tempo * Math.PI * 2 + phase * 6.28);
      ...
      const alpha = clamp((0.1 + 0.3 * flicker) * (0.25 + 0.75 * wave) * revealAlpha, 0, 1);
