...
class DsEffortSlider extends HTMLElement {
  ...
  // High(3)/Extra(4) 的弱化粒子场：稀疏点阵 + 明暗水波纹。
  // 明暗波从 thumb 中心向外扩散（粒子亮度随半径+时间起伏），
  // 强度比 Max 弱很多，颜色区分（High 蓝 / Extra 紫）。
  _drawRippleField(context, width, height, time, mode) {
    ...
    // 明暗水波纹：3 条亮带随相位从中心向外扩散
    const WAVE = 3;
    const ripplePeriod = 2200;
    ...
    for (let row = 0; row < rows; row += 1) {
      for (let column = 0; column < columns; column += 1) {
        ...
        const dist = clamp(1 - dx, 0, 1); // 中心亮，向两侧衰减
        ...
        const alpha = dist * (0.13 + 0.2 * wave);
